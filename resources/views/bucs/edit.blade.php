...
<div class="box box-default">
  <div class="box-header with-border">
    ...
    <h3 class="box-title">Edit BUC</h3>
  </div>
   ...
   <div class="box-body">
      ...
      <div class="row">
        <div class="col-md-6 form-group @error('nomor') has-error @enderror">
          ...
          <input type="text" name="nomor" class="form-control" id="nomor" value="{{ $buc->nomor }}" readonly>
          @error('nomor')
        ...
        <div class="col-md-4 form-group @error('start_date') has-error @enderror">
          ...
          <input type="date" name="start_date" class="form-control" id="start_date" placeholder="Misal: pembangunan pos security" value="{{ old('start_date', $buc->start_date) }}">
          @error('start_date')
            <div class="help-block">
              {{ $message }}
            </div>
      ...
      <div class="row">
        <div class="col-md-12 form-group @error('activity_name') has-error @enderror">
          ...
          <input type="text" name="activity_name" class="form-control" id="activity_name" placeholder="Misal: pembangunan pos security" value="{{ old('activity_name', $buc->activity_name) }}">
          @error('activity_name')
            <div class="help-block">
              {{ $message }}
            </div>
          ...
          <select name="project_code" id="project_code" class="form-control">
            ...
            @if ($projects)
                @foreach ($projects as $project)
                  <option value="{{ $project['kode_project'] }}" {{ $project['kode_project'] == $buc->project_code ? 'selected' : '' }}>{{ $project['kode_project'] }}</option>
                @endforeach
            @else
        ...
        <div class="col-md-6 form-group @error('person_incharge_id') has-error @enderror">
          ...
          <select name="person_incharge_id" id="person_incharge_id" class="form-control">
            ...
            @foreach ($pincharges as $pincharge)
              <option value="{{ $pincharge->id }}" {{ $pincharge->id == $buc->person_incharge_id ? 'selected' : '' }} >{{ $pincharge->name }}</option>
            @endforeach
          </select>
        </div>
      </div> {{-- end row --}}
